...
func TestAddJWTSecrets(t *testing.T) {
	...
	tests := []struct {
		policies        []*conf_v1alpha1.Policy
		expectedSecrets map[string]*configs.SecretReference
		wantErr         bool
		msg             string
	}{
		{
	...
	for _, test := range tests {
		...
		err := lbc.addJWTSecrets(result, test.policies)
		if (err != nil) != test.wantErr {
			t.Errorf("addJWTSecrets() returned %v, for the case of %v", err, test.msg)
		}

		if diff := cmp.Diff(test.expectedSecrets, result, cmp.Comparer(errorComparer)); diff != "" {
			t.Errorf("addJWTSecrets() '%v' mismatch (-want +got):\n%s", test.msg, diff)
		}
	}
}
...
func TestAddIngressMTLSSecret(t *testing.T) {
	...
	for _, test := range tests {
		...
		err := lbc.addIngressMTLSSecret(result, test.policies)
		if (err != nil) != test.wantErr {
			t.Errorf("addIngressMTLSSecret() returned %v, for the case of %v", err, test.msg)
		}

		if diff := cmp.Diff(test.expectedSecrets, result, cmp.Comparer(errorComparer)); diff != "" {
			t.Errorf("addIngressMTLSSecret() '%v' mismatch (-want +got):\n%s", test.msg, diff)
		}
	}
}
...
func TestAddEgressMTLSSecrets(t *testing.T) {
	...
	for _, test := range tests {
		...
		err := lbc.addEgressMTLSSecrets(result, test.policies)
		if (err != nil) != test.wantErr {
