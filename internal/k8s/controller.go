...
func (lbc *LoadBalancerController) createIngressEx(ing *networking.Ingress, validHosts map[string]bool, validMinionPaths map[string]bool) *configs.IngressEx {
	...
	ingEx.Secrets = make(map[string]*configs.SecretReference)
	...
	for _, tls := range ing.Spec.TLS {
		...
		ingEx.Secrets[secretName] = &configs.SecretReference{
			Type:  secretType,
			Path:  secretPath,
			Error: err,
		}
...
func (lbc *LoadBalancerController) createVirtualServerEx(virtualServer *conf_v1.VirtualServer, virtualServerRoutes []*conf_v1.VirtualServerRoute) *configs.VirtualServerEx {
	virtualServerEx := configs.VirtualServerEx{
		...
		Secrets:       make(map[string]*configs.SecretReference),
	}
	...
	if virtualServer.Spec.TLS != nil && virtualServer.Spec.TLS.Secret != "" {
		...
		virtualServerEx.Secrets[secretKey] = &configs.SecretReference{
			Type:  secretType,
			Path:  secretPath,
			Error: err,
		}
	...
	for _, r := range virtualServer.Spec.Routes {
		...
		err = lbc.addJWTSecrets(virtualServerEx.Secrets, vsRoutePolicies)
		if err != nil {
			glog.Warningf("Error getting JWT secrets for VirtualServer %v/%v: %v", virtualServer.Namespace, virtualServer.Name, err)
		}
	}
	...
	for _, vsr := range virtualServerRoutes {
		for _, sr := range vsr.Spec.Subroutes {
			...
			err = lbc.addJWTSecrets(virtualServerEx.Secrets, vsrSubroutePolicies)
			if err != nil {
...
func (lbc *LoadBalancerController) addJWTSecrets(secrets map[string]*configs.SecretReference, policies []*conf_v1alpha1.Policy) error {
	for _, pol := range policies {
		...
		secrets[secretKey] = &configs.SecretReference{
			Type:  secretType,
			Path:  secretPath,
			Error: err,
		}
...
func (lbc *LoadBalancerController) addIngressMTLSSecret(secrets map[string]*configs.SecretReference, policies []*conf_v1alpha1.Policy) error {
	for _, pol := range policies {
...
func (lbc *LoadBalancerController) addEgressMTLSSecrets(secrets map[string]*configs.SecretReference, policies []*conf_v1alpha1.Policy) error {
	for _, pol := range policies {
