...
func TestGenerateNginxCfgForJWT(t *testing.T) {
	...
	cafeIngressEx.Secrets["cafe-jwk"] = &SecretReference{
		Type: "nginx.org/jwk",
...
func TestGenerateNginxCfgWithMissingTLSSecret(t *testing.T) {
	cafeIngressEx := createCafeIngressEx()
	cafeIngressEx.Secrets["cafe-secret"].Error = errors.New("secret doesn't exist")
	configParams := NewDefaultConfigParams()
...
func TestGenerateNginxCfgForMergeableIngressesForJWT(t *testing.T) {
	...
	mergeableIngresses.Master.Secrets["cafe-jwk"] = &SecretReference{
		Type: SecretTypeJWK,
		Path: "/etc/nginx/secrets/default-cafe-jwk",
	}
	...
	mergeableIngresses.Minions[0].Secrets["coffee-jwk"] = &SecretReference{
		Type: SecretTypeJWK,
...
func TestAddSSLConfig(t *testing.T) {
	...
	for _, test := range tests {
		...
		addSSLConfig(&result, test.host, test.tls, test.secrets, test.isWildcardEnabled)
...
func TestGenerateJWTConfig(t *testing.T) {
	...
	for _, test := range tests {
		jwtAuth, redirectLocation := generateJWTConfig(test.secrets, test.cfgParams, test.redirectLocationName)
