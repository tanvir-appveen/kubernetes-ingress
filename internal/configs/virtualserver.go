...
func (vsc *virtualServerConfigurator) GenerateVirtualServerConfig(vsEx *VirtualServerEx) (version2.VirtualServerConfig, Warnings) {
	...
	sslConfig := generateSSLConfig(vsEx.VirtualServer.Spec.TLS, vsEx.VirtualServer.Namespace, vsEx.Secrets, vsc.cfgParams)
	tlsRedirectConfig := generateTLSRedirectConfig(vsEx.VirtualServer.Spec.TLS)

	policyOpts := policyOptions{
		tls:     sslConfig != nil,
		secrets: vsEx.Secrets,
	}
...
func (p *policiesCfg) addEgressMTLSConfig(
	...
	if egressMTLS.TLSSecret != "" {
		...
		tlsSecret := secrets[egressTLSSecret]
	...
	if egressMTLS.TrustedCertSecret != "" {
		...
		trustedSecret := secrets[trustedCertSecret]
	...
	for _, p := range policyRefs {
		...
		if pol, exists := policies[key]; exists {
			...
			case pol.Spec.JWTAuth != nil:
				res = config.addJWTAuthConfig(pol.Spec.JWTAuth, key, polNamespace, policyOpts.secrets)
			...
			case pol.Spec.EgressMTLS != nil:
				res = config.addEgressMTLSConfig(pol.Spec.EgressMTLS, key, polNamespace, policyOpts.secrets)
			default:
				res = newValidationResults()
			}
...
func generateSSLConfig(tls *conf_v1.TLS, namespace string, secrets map[string]*SecretReference, cfgParams *ConfigParams) *version2.SSL {
	if tls == nil {
		return nil
	}
	...
	secret := secrets[fmt.Sprintf("%s/%s", namespace, tls.Secret)]
